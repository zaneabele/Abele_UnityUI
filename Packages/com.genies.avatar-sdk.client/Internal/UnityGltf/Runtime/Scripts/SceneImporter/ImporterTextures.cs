			...
			switch (image.MimeType)
			{
				...
				case "image/ktx2":
					...
					if (Context.TryGetPlugin<Ktx2ImportContext>(out _))
					{
						...
						var resultTextureData = await ktxTexture.LoadFromBytes(new Unity.Collections.NativeArray<byte>(data, Unity.Collections.Allocator.Temp).AsReadOnly(), isLinear, mipChain: GenerateMipMapsForTextures);
